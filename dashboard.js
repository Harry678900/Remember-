...
class BirthdayDashboard {
            ...
            <div style="padding: 15px; border-bottom: 1px solid #eee;">
                ...
                ${notifications.map(notification => `
                    <div style="padding: 10px; background: #f8f9fa; border-radius: 6px; margin-bottom: 8px; border-left: 3px solid ${color};">
                        <p style="margin: 0; color: #333; font-size: 14px;">${notification.message}</p>
                    </div>
                `).join('')}
            </div>
        `;
    ...
    checkBirthdaysToday() {
        ...
        todayBirthdays.forEach(birthday => {
            ...
            // Only show popup/notification if we haven't shown it today for this birthday
            if (!cleanedShownToday[birthdayKey]) {
                // Mark as shown
                ...
                // Show birthday popup (WhatsApp/Gmail) after a short delay
                setTimeout(() => {
    ...
    checkAndSendUpdatedBirthdayNotification(updatedBirthday) {
        ...
        if (isToday) {
            ...
            // Mark as shown (even if it was shown before, we want to show it again after update)
            shownToday[birthdayKey] = true;
